...
class Algorithm(BaseAlgorithm):
    def fit_predict(self, data: dict[str, pd.DataFrame]) -> tuple[np.ndarray, np.ndarray, list[str]]:
        ...
        if not omics_path:
            raise ValueError("NEMO requires omics_path so R can read the saved omics_data.parquet directly.")

        input_path = Path(omics_path)
        if not input_path.exists():
            raise FileNotFoundError(f"NEMO input file not found: {input_path}")

        script_path = self._script_in_algorithm_dir(__file__)
        if not script_path.exists():
            raise FileNotFoundError(f"NEMO R script not found: {script_path}")

        n_clusters = int(self.params.get("n_clusters", 3))
        n_neighbors = int(self.params.get("n_neighbors", 0))
        ...
        try:
            ...
            raise RuntimeError(
                f"NEMO exceeded the {int(self.params.get('timeout', 3600))} second timeout."
            ) from exc
        ...
        if labels.ndim != 1:
            raise ValueError("NEMO returned labels with an invalid shape.")
        if embeddings.ndim != 2 or embeddings.shape[1] < 1:
            raise ValueError("NEMO returned embeddings with an invalid shape.")
        if len(sample_names) != len(labels) or embeddings.shape[0] != len(labels):
            raise ValueError("NEMO returned inconsistent sample, label, and embedding counts.")
        if not np.isfinite(embeddings).all():
            raise ValueError("NEMO returned missing or non-finite embedding values.")
    ...
    def _parse_r_output(self, result: subprocess.CompletedProcess[str]) -> dict:
        ...
        if result.returncode != 0:
            message = payload.get("error") or self._format_r_failure("NEMO R script failed.", stdout, stderr)
        ...
        if not payload:
            raise RuntimeError(self._format_r_failure("NEMO R script did not return JSON output.", stdout, stderr))
