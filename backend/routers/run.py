...
class AnalysisRequest(BaseModel): #定义数据校验模型 #定义一个类，在这个类里声明几个变量，并且明确指定这几个变量的类型。以此实现确保输入数据类型符合该要求，如果不符合，FastAPI会自动拦截并返回422错误
    ...
    # 【新增】谱聚类的核心参数：邻居数
    n_neighbors: int=10
    # 【新增】各内置算法的可自定义参数（均带默认值，保持向后兼容；各 wrapper 用 self.params.get 读取）
    n_init: int=10                       # K-means / Spectral：初始化次数
    tol: float=1e-4                      # K-means：收敛阈值
    init: str="k-means++"               # K-means：初始化方式（k-means++ / random）
    assign_labels: str="kmeans"         # Spectral：标签分配方式（kmeans / discretize / cluster_qr）
    hclust_method: str="ward.D2"        # Hclust：连接方式
    hclust_distance: str="euclidean"    # Hclust：距离度量
    snf_alpha: float=0.5                 # SNF：高斯核参数 σ(alpha)
    snf_iterations: int=20               # SNF：扩散迭代次数 T
    latent_dim: int=2                    # PIntMF：隐变量维度（前端默认随 K）
    pintmf_max_iter: int=5               # PIntMF：最大迭代
    pintmf_max_features: int=500         # PIntMF：每个组学保留的最大特征数
    nemo_n_neighbors: int=0              # NEMO：邻居数（0=自动），独立字段避免与 Spectral/SNF 冲突
    parea_structure: str="2"            # Parea：集成结构（2 / 1）
    #用户选择的降维算法
...
async def run_analysis(request:AnalysisRequest): #指定record的类型为AnalysisRequest，就是我们刚才定义的那个类，如果前端传来的数据类型不匹配，FastAPI会自动拦截并返回422错误
    ...
    try:
        ...
        # NEMO 用独立的 nemo_n_neighbors（默认 0=自动），其余算法用通用的 n_neighbors
        n_neighbors = request.nemo_n_neighbors if request.algorithm == "NEMO" else request.n_neighbors
        algo_instance = algo_class(
            n_clusters=request.n_clusters,
            random_state=seed,
            max_iter=request.max_iter,
            n_neighbors=n_neighbors,
            # 以下为各算法的可自定义参数，多传不影响其它算法（wrapper 用 self.params.get 按需读取）
            n_init=request.n_init,
            tol=request.tol,
            init=request.init,
            assign_labels=request.assign_labels,
            hclust_method=request.hclust_method,
            hclust_distance=request.hclust_distance,
            snf_alpha=request.snf_alpha,
            snf_iterations=request.snf_iterations,
            latent_dim=request.latent_dim,
            pintmf_max_iter=request.pintmf_max_iter,
            pintmf_max_features=request.pintmf_max_features,
            parea_structure=request.parea_structure,
...
class ParameterSearchRequest(BaseModel):
    ...
    random_state: int = 42
    # 普通模式里为该算法设置的枚举/标量参数（如 Hclust method、SNF alpha），在扫描时作为固定参数生效
    fixed_params: dict = {}
...
async def run_parameter_search(request: ParameterSearchRequest):
    try:
        ...
        for combo in combinations:
            ...
            # 固定参数打底，被扫描的参数覆盖之，最后固定随机种子
            params_with_seed = {**request.fixed_params, **params, "random_state": request.random_state}
            try:
